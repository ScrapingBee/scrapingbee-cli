...
class TestExportDiffDir:
    """Tests for export --diff-dir change detection."""

    def test_unchanged_items_skipped(self, tmp_path):
        old_dir = tmp_path / "old"
        new_dir = tmp_path / "new"
        old_dir.mkdir()
        new_dir.mkdir()
        content = json.dumps({"title": "Widget", "price": 9.99})
        (old_dir / "1.json").write_text(content)
        (new_dir / "1.json").write_text(content)  # same content

        runner = CliRunner()
        result = runner.invoke(
            cli,
            ["export", "--input-dir", str(new_dir), "--diff-dir", str(old_dir)],
        )
        assert result.exit_code != 0  # no changed items → no output files → exits non-zero

    def test_changed_items_included(self, tmp_path):
        old_dir = tmp_path / "old"
        new_dir = tmp_path / "new"
        old_dir.mkdir()
        new_dir.mkdir()
        (old_dir / "1.json").write_text(json.dumps({"title": "Old"}))
        (new_dir / "1.json").write_text(json.dumps({"title": "New"}))  # changed
    ...
    def test_new_items_included(self, tmp_path):
        old_dir = tmp_path / "old"
        new_dir = tmp_path / "new"
        old_dir.mkdir()
        new_dir.mkdir()
        (new_dir / "1.json").write_text(json.dumps({"title": "Brand new"}))
        # old_dir has no file 1.json → item is new
    ...
    def test_mixed_changed_and_unchanged(self, tmp_path):
        ...
        old_dir.mkdir()
        new_dir.mkdir()
        same = json.dumps({"title": "Same"})
        (old_dir / "1.json").write_text(same)
        (new_dir / "1.json").write_text(same)  # unchanged
        (new_dir / "2.json").write_text(json.dumps({"title": "Changed"}))  # new
